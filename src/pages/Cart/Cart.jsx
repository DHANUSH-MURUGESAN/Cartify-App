...
const Cart = () => {
  ...
  {cart.length === 0 ? (
    <p className="text-center text-lg text-slate">No items in your cart.</p>
  ) : (
    <div>
      {/* Items in cards */}
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {cart.map((item) => (
          <div
            key={item._id}
            className="bg-white shadow-lg rounded-xl p-4 flex flex-col justify-between border border-gray-light"
          >
            <img
              src={item.image}
              alt={item.title}
            ...
            <div className="flex-1">
              <h2 className="font-bold text-lg mb-1 text-dark">{item.title}</h2>
              <p className="text-slate">₹ {item.price} (per item)</p>

              {/* Quantity controls */}
              <div className="flex items-center gap-3 mt-3">
                <button
                  onClick={() => decreaseQuantity(item._id)}
                  className="px-3 py-1 bg-gray-light text-dark rounded-lg hover:bg-mint hover:text-white transition"
                >
                  -
                </button>
            ...
            <button
              ...
              className="mt-4 bg-sky text-white px-4 py-2 rounded-lg font-semibold hover:bg-amber transition duration-300"
            >
              Remove
            </button>
          </div>
      ...
      <div className="text-right mt-10">
        ...
        <button
          ...
          className="bg-amber text-dark px-6 py-3 rounded-2xl font-semibold hover:bg-mint hover:text-white transition duration-300"
        >
          Proceed to Checkout
        </button>
      </div>
